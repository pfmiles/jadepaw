...
- ✓ **REQ-AGENT-002**: 工具调用能力，MCP 兼容协议，文件读写 + HTTP 请求 — Phase 4
- ✓ **REQ-SECURITY-004**: 网络访问控制（白名单域名 + SSRF 防护） — Phase 4
...
| MCP 兼容工具协议 | 与主流 Agent 生态互通，降低工具开发成本 | Validated — Phase 4 |
...
| HostFunctions trait (async_trait) | jadepaw-core 中定义，additive-only 设计，Phase 4 扩展 http_request + SSRF 防护 | Validated — Phase 2, 4 |
| SessionRepository trait + SQLite persistence | jadepaw-db crate, WAL mode, session_id+tenant_id 强制隔离, turn-boundary checkpoint | Validated — Phase 5 |
| 上下文窗口自动压缩（65% 阈值） | tiktoken-rs token 计数, 旧轮次摘要 + 最近 N=5 轮保留, 每次 LLM 调用前检查 | Validated — Phase 5 |
| Session pause/resume + crash recovery | 全状态快照至 SQLite, resume 时创建新 Wasm Store, mark_running_as_paused crash recovery | Validated — Phase 5 |
...
---
*Last updated: 2026-06-03 after Phase 4*