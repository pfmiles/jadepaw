...
- [ ] **REQ-AGENT-001**: 混合 Agent Loop（粗粒度规划 + ReAct 执行），支持计划偏离时局部重规划
...
|----------|-----------|---------|
| Wasm + Rust + tokio 技术栈 | 线性内存隔离 + 编译时安全 + 高性能异步 | — Pending |
| 预初始化实例池 + 热状态注入 | 冷启动 ≤5ms，支持超高并发密度 | — Pending |
...
| 能力安全模型（默认拒绝） | 多租户环境下必须零信任，所有能力显式授权 | — Pending |
