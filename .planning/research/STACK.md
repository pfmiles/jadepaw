# Stack Research: Jadepaw AI Agent Runtime
...
cargo add wasmtime@38.0 wasmtime-wasi@38.0
cargo add tokio@1.43 -F full
cargo add axum@0.8.4 -F ws
cargo add async-openai@0.34.0 -F _default

# Database & caching
cargo add sqlx@0.8 -F runtime-tokio-rustls,postgres,sqlite
cargo add redis@0.28 -F tokio-comp,tokio-native-tls-comp -F connection-manager

# Observability
cargo add tracing@0.1 tracing-subscriber@0.3 -F env-filter,json
cargo add opentelemetry@0.28 opentelemetry-otlp@0.28 -F http-proto,metrics
cargo add tracing-opentelemetry@0.28
cargo add metrics@0.24 metrics-exporter-prometheus@0.16
...
cargo add tower-http@0.6 -F cors,compression-gzip,trace,fs
cargo add uuid@1.0 -F v7
cargo add chrono@0.4 -F serde

# Dev dependencies
cargo add --dev rstest@0.24 axum-test@0.17
cargo add --dev testcontainers@0.24
cargo add --dev tower@0.5 -F util
```
...
```toml
[profile.release]
lto = "fat"           # Maximize Wasm JIT optimization
...
**If single-node deployment (<1000 instances):**
...
**If cluster deployment (1000+ instances across nodes):**
...
**If the LLM provider is self-hosted (Ollama, vLLM, local Llama.cpp):**
...
**If the LLM provider is cloud (OpenAI, Azure, Anthropic via proxy):**
...
**wasmtime version pinning**: wasmtime 38.x uses Cranelift as the default JIT backend. Do NOT use 37.x or below -- significant improvements to the pooling allocator and WASI preview2 component model support were added. Pin exact minor version in `Cargo.toml` and bump deliberately after reviewing release notes, especially the security advisories. The Bytecode Alliance publishes CVEs through GitHub Security Advisories and the `sec-announce@bytecodealliance.org` mailing list.

**Rust edition 2024**: Required per project constraints. Ensure all crates compile cleanly on edition 2024. Most major crates (tokio, axum, wasmtime, sqlx, async-openai) already support it.
...
---
*Stack research for: jadepaw multi-tenant AI Agent runtime with WebAssembly isolation*
*Researched: 2026-05-28*